...
- Importar y describir bases de datos.

- Tomar decisiones epidemiológicas y estadísticas en base a la pregunta y tipo de estudio.

## Preguntas de investigación de hoy

1. ¿Cuántas veces es el odds de tener una enfermedad cardiovascular en personas obesas?

2. ¿Cuántas veces es la prevalencia de hipercolesterolemia en personas con actividad física leve?

3. ¿Cuántas veces es el riesgo de desarrollar SIDA dado que poseeo el polimorfismo SDF1(CXCR12)?

4. ¿Las personas con el polimorfismo SDF1(CXCR12) viven más tiempo sin SIDA?
...
```{r}
chron <- read_dta("data-raw/chronic2.dta") %>% as_factor() %>% 
...
```{r}
#contexto:
#y -> derpleu: derrame pleural en el último mes
#x -> rx: tratamiento DPCA vs convencional
#for poisson:
# - y must be numeric
# - be sure that is coded as 0 or 1 if outcome is dicotomic.
noconv <- read_dta("data-raw/noconveregence.dta") %>% as_factor() %>% 
  mutate(derpleu1_n=as.numeric(derpleu1),
         derpleu1_nn=if_else(derpleu1 == "No",0,1))

noconv %>% glimpse()
noconv %>% count(derpleu1,derpleu1_n,derpleu1_nn)

glm(derpleu1 ~ rx + sex + age5 + asictes + hepatom + spiders_ + edema_ + ln_bilirubin + ln_albumin + histol_, 
    data = noconv, family = binomial(link = "log"))
