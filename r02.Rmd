...
## 3.3 Gererar tabla Kaplan Meier
...
## 3.3 Gererar gráfico e interpretar (1)
...
## 3.3 Gererar gráfico e interpretar (2)
...
$$ OR \Longrightarrow logit(\pi) = log(odds)  = log\left(\frac{\pi}{1-\pi}\right) = Y = \beta_0 + \beta_1 X_1 + . . . + \beta_n X_n + \epsilon \enspace ; \enspace y \sim Binomial $$

- El valor exponenciado de los coeficientes se pueden interpretar como **Odds Ratio (OR)**

$$ Y = \beta_0 + \beta_1 X_1 \\ \begin{cases} log(odds_2) = \beta_0 + \beta_1(1) \\ log(odds_1) = \beta_0 + \beta_1(0) \end{cases} \\ log(odds_2) - log(odds_1) = \beta_1 \\ log \left(\frac{odds_2}{odds_1}\right)  = \beta_1 \\ OR = exp(\beta_1) $$
...
- **RR o PR:** **familia:** binomial, **link:** log

$$ RR \Longrightarrow log(\pi) = log(\pi) =  \beta_0 + \beta_1 X_1 + . . . + \beta_n X_n + \epsilon \enspace ; \enspace y \sim Binomial $$

- El valor exponenciado de los coeficientes se pueden interpretar como **Risk Ratio (RR)**

$$ Y = \beta_0 + \beta_1 X_1 \\ \begin{cases} log(p_2) = \beta_0 + \beta_1(1) \\ log(p_1) = \beta_0 + \beta_1(0) \end{cases} \\ log(p_2) - log(p_1) = \beta_1 \\ log \left(\frac{p_2}{p_1}\right)  = \beta_1 \\ RR = exp(\beta_1) $$
...
**En personas VIH+, ajustando por raza, CCR2, SDF1 y edad, ¿el polimorfismo CCR5(Delta-32) *protege* contra el desarrollo del SIDA?**
...
```{r}
#alternativa: poisson
wm1 <- glm(tcho2_nn ~ age + sex + group + bmi_cat + diab + hba1c + ht + pa2 + smoke2 + alcohol2 + ndrug, 
    data = cronic, 
...
- Situación 1: Genera un modelo múltiple tomando en cuenta `estatura` como confusor
...
- Regresión de Cox (Hazard Ratio): `http://www.sthda.com/english/wiki/cox-proportional-hazards-model`
